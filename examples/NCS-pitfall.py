...
class NCSAlgo(object):
    def __init__(self, **kwargs):
        ...
        if self.args['env_type'] == 'atari':
            ...
            # self.policy = Policy(env, network=self.args['network'],
            #  nonlin_name=self.args['nonlin_name']) 
            with open('human_buf.pickle','rb+') as f:
                humanBuf = pickle.load(f)
                self.buflength = humanBuf.length

            self.policy = SupervisePolicy(
                env,network=self.args['network'],nonlin_name=self.args['nonlin_name'], humanBuf = humanBuf
            )
    ...
    def firstEvaluation(self):
        ...
        if self.args['loadpath'] is not None:
            self.param = self.logger.load_parameters(self.args['loadpath']).copy()
    ...
    def syncOneValue(self, v):
        ...
        # self.logger.log('v_t:'+str(v_t))
        self.comm.Allgather([v_t, MPI.DOUBLE], [v_all, MPI.DOUBLE])
        # self.logger.log('v_all:'+str(v_all))
    ...
    def generateAndEvalChild(self):
        ...
        if self.rank != 0:
            ...
            # self.logger.log('steps_cost_child'+str(steps_cost_child))
        ...
        # self.logger.log('tmp_steps_cost'+str(tmp_steps_cost))
        cost_steps += np.sum(tmp_steps_cost)
        # self.logger.log('cost step'+str(cost_steps))
        return cost_steps
